...
void MatchMakingServer::process_data(ENetEvent& event) {
    ...
    if (type == MessageType::lobby_list_update) {
        ...
        launch_lobby(*lobby, info, event.peer);
...
void MatchMakingServer::launch_lobby(server_lobby_t& lobby, const GameServerInfo& server, ENetPeer* server_peer) {
    lobby.state = LobbyState::playing;
    for (auto& player : lobby.players) {
        auto secret = generate_secret();
        // Send command to the player
        {
            OutByteStream msg;
            msg << MessageType::lobby_start << server << secret;
            spdlog::info("lanching client on port {}", player.peer->address.port);
            while (send_bytes<true>(msg.get_span(), player.peer) != 0) { }
        }
        // Send player secret to the game server
        {
            OutByteStream msg;
            msg << MessageType::secret << player.name << secret;
            while (send_bytes<true>(msg.get_span(), server_peer) != 0) { }
        }
...
secret_t<s_secret_size> MatchMakingServer::generate_secret() {
    auto& gen = get_random_generator();
    std::uniform_int_distribution<uint8_t> distr{1, 6};
    secret_t<s_secret_size> result;
    std::ranges::generate(result, [&]{return std::byte(distr(gen));});
    return result;
}
